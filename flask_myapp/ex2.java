...
static int mytspdp(int c[][], int tour[], int start, int n)
{
vizfound=false;
vizcount++;
...
st.push(vizcount);
int vizret=tspdp(c,tour,start,n);
st.pop();
...
{
if(vizfound){int vizret=mytspdp(c,tour,start,n);
...
if (c[tour[start]][tour[i]] + (cost = mytspdp(c, temp, start + 1,n)) < mincost)
{
